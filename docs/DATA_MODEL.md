...
## Media Asset Entity Model

### 1. Purpose

The Media Asset entity represents a reusable digital file used throughout the Star Furniture Goa platform.

A Media Asset represents the file itself, such as an image, PDF, video, icon, logo, catalogue document, swatch, gallery image, or marketing asset.

Business entities reference Media Assets through Media References.

A Media Asset must not own Products, Categories, Collections, Catalogues, Company, Brand, Location, or any other business entity.

---

### 2. Responsibilities

The Media Asset entity is responsible for:

- Representing reusable digital files.
- Storing provider-independent file identity and storage information.
- Supporting images, PDFs, videos, icons, logos, thumbnails, and future immersive media.
- Supporting cloud storage and CDN delivery.
- Supporting optimized image variants and responsive media.
- Supporting thumbnail generation.
- Supporting accessibility metadata.
- Supporting SEO-related media metadata.
- Supporting file metadata such as format, dimensions, size, and duration.
- Supporting versioning and future Digital Asset Management workflows.

The Media Asset entity is not responsible for:

- Owning business entities.
- Deciding where media appears on the website.
- Defining Product, Category, Collection, Catalogue, Company, Brand, or Location content.
- Storing page layout rules.
- Storing hardcoded local file paths.
- Implementing cloud storage logic.
- Implementing image optimization logic.
...
| Name | Data Type | Required or Optional | Description | Validation Rules |
|---|---|---|---|---|
| `id` | UUID String | Required | Internal unique identifier for the Media Asset. | Must be globally unique and immutable. |
| `companyId` | UUID String | Required | Reference to the Company that owns or manages the Media Asset. | Must reference one valid Company. |
| `name` | String | Required | Human-readable asset name. | Must be clear, descriptive, and unique enough for asset management. |
| `assetType` | Controlled String | Required | Broad type of digital asset. | Suggested values: `image`, `pdf`, `video`, `icon`, `logo`, `document`, `audio`, `threeSixtyView`, `virtualTour`, `other`. |
| `mimeType` | String | Required | Technical media type of the file. | Must use a valid MIME type, such as `image/webp`, `image/png`, `application/pdf`, or `video/mp4`. |
| `storageReference` | Object | Required | Provider-independent reference to where the file is stored. | Must not be a raw local file path. Must support cloud storage and CDN delivery. |
| `status` | Controlled String | Required | Lifecycle status of the Media Asset. | Suggested values: `active`, `inactive`, `draft`, `archived`, `processing`, `failed`, `deleted`. |
| `createdAt` | ISO 8601 Date String | Required | Date and time when the Media Asset was created. | Must use ISO 8601 format. |
| `updatedAt` | ISO 8601 Date String | Required | Date and time when the Media Asset was last updated. | Must use ISO 8601 format and update whenever Media Asset data changes. |
...
| Name | Data Type | Required or Optional | Description | Validation Rules |
|---|---|---|---|---|
| `publicId` | String | Optional | Customer-safe identifier for public or external references. | Must not expose internal sequencing or sensitive storage details. |
| `slug` | String | Optional | URL-safe identifier for assets that may be publicly addressable. | Must be lowercase and hyphen-separated when used. |
| `description` | String | Optional | Human-readable explanation of the asset. | Must describe the digital asset itself, not a business entity using it. |
| `fileName` | String | Optional | Original or managed file name. | Should be descriptive and sanitized. |
| `fileExtension` | String | Optional | File extension such as `webp`, `png`, `pdf`, or `mp4`. | Must match the file type where possible. |
| `fileSizeBytes` | Number | Optional | Size of the original file in bytes. | Must be a positive number when present. |
| `checksum` | String | Optional | File integrity hash. | Should be generated by the storage or asset management process. |
| `storageProvider` | Controlled String | Optional | Storage provider used for the asset. | Suggested values: `local`, `cloudflareR2`, `amazonS3`, `cms`, `external`, `other`. |
| `cdnUrl` | String | Optional | CDN delivery URL if available. | Must be a valid URL and must not replace `storageReference`. |
| `sourceUrl` | String | Optional | External source URL if the asset is externally hosted. | Must be a valid URL when used. |
| `altText` | String | Optional | Accessibility text for images and visual assets. | Required for meaningful customer-facing images. Should be concise and descriptive. |
| `caption` | String | Optional | Optional display caption for the asset. | Must describe the asset without duplicating entity content. |
| `creditText` | String | Optional | Photographer, creator, or rights credit. | Should be used where attribution is required. |
| `usageRights` | Object | Optional | Rights, license, or permission information. | Should describe usage permissions and restrictions when relevant. |
| `width` | Number | Optional | Width in pixels for visual assets. | Must be a positive number when present. |
| `height` | Number | Optional | Height in pixels for visual assets. | Must be a positive number when present. |
| `aspectRatio` | String | Optional | Aspect ratio for visual assets. | Should use a consistent format such as `16:9`, `4:3`, or `1:1`. |
| `durationSeconds` | Number | Optional | Duration for video or audio assets. | Must be a positive number when present. |
| `pageCount` | Number | Optional | Page count for PDF or document assets. | Must be a positive number when present. |
| `colorProfile` | String | Optional | Color profile for image assets. | Should be used only when relevant for visual quality control. |
| `dominantColor` | String | Optional | Approximate dominant color for loading states or visual grouping. | Should use a valid color value when present. |
| `responsiveVariants` | Array of Objects | Optional | Optimized versions of the asset for different sizes or formats. | Must reference provider-independent storage locations and must not use raw local paths. |
| `thumbnailMediaAssetId` | UUID String | Optional | Reference to a generated thumbnail Media Asset. | Must reference a valid Media Asset. |
| `derivativeMediaAssetIds` | Array of UUID Strings | Optional | References to optimized, cropped, compressed, or generated asset variants. | Must reference valid Media Assets. |
| `relatedMediaAssetIds` | Array of UUID Strings | Optional | References to related assets such as alternate crops or companion files. | Must reference valid Media Assets. |
| `assetCategory` | Controlled String | Optional | Business grouping for asset management. | Suggested values: `product`, `category`, `collection`, `catalogue`, `brand`, `company`, `location`, `material`, `finish`, `marketing`, `icon`, `logo`, `other`. |
| `tags` | Array of Strings | Optional | Asset management tags. | Should use normalized terms where possible. |
| `searchKeywords` | Array of Strings | Optional | Searchable terms for finding the asset internally. | Should support asset discovery without duplicating business entity data. |
| `seoMetadata` | SEO Metadata Object | Optional | SEO metadata for media assets that may be indexed or shared. | Must use the shared SEO Metadata value object. |
| `visibilitySettings` | Visibility Settings Object | Optional | Controls asset visibility in customer-facing or CMS-managed contexts. | Must use the shared Visibility Settings value object. |
| `localizations` | Array of Localization Text Objects | Optional | Future translations for asset captions, alt text, or descriptions. | Must follow the approved localization strategy. |
| `versionInformation` | Version Information Object | Optional | Tracks revisions or replacements of the Media Asset. | Must follow the approved versioning strategy when used. |
| `metadata` | Object | Optional | Non-critical extensibility data. | Must not contain business-critical ownership relationships or raw storage secrets. |
| `isDeleted` | Boolean | Optional | Soft deletion flag. | Must default to `false` when used. |
| `deletedAt` | ISO 8601 Date String | Optional | Date and time when the Media Asset was soft deleted. | Required only when `isDeleted` is `true`. |

---

### 5. Relationships

- Media Asset belongs to one Company through `companyId`.
- Business entities reference Media Assets through Media References.
- Product may reference Media Assets through `mediaReferences`.
- Product Variant may reference Media Assets through `mediaReferences`.
- Category may reference Media Assets through `mediaReferences`.
- Collection may reference Media Assets through `mediaReferences`.
- Catalogue may reference Media Assets through `primaryMediaReference`, `coverMediaReference`, `thumbnailMediaReference`, or `mediaReferences`.
- Material may reference Media Assets through `mediaReferences`.
- Finish may reference Media Assets through `mediaReferences`.
- Brand may reference Media Assets through logo, icon, and brand media references.
- Company and Location may reference Media Assets through `mediaReferences`.
- Media Asset may reference another Media Asset through `thumbnailMediaAssetId`, `derivativeMediaAssetIds`, or `relatedMediaAssetIds`.

Media Asset does not store `productIds`, `categoryIds`, `collectionIds`, `catalogueIds`, `brandIds`, `locationIds`, or other reverse ownership references.

---

### 6. Business Rules

- A Media Asset must represent a digital file only.
- A Media Asset must not own business entities.
- Business entities must reference Media Assets through Media References.
- Media Asset storage must be provider-independent.
- Media Asset must not store raw local file paths.
- Storage details must not expose private credentials, secrets, or implementation-specific access rules.
- Catalogue PDFs must be represented as Media Assets and referenced from Catalogue through Media References.
- Product galleries must reference Media Assets through Product or Product Variant media references.
- Logos and icons may be Media Assets but must not cause Media Asset to own Brand identity.
- Customer-facing meaningful images should include `altText`.
- Decorative assets may omit `altText` only when they are not meaningful content.
- Responsive image variants should be represented as optimized variants or derivative Media Assets.
- Thumbnail generation should reference derived Media Assets rather than duplicating file data.
- A Media Asset may be reused across multiple business entities.
- A Media Asset should remain available for historical references unless intentionally archived or deleted.
- Soft-deleted Media Assets must not be used in public experiences unless explicitly restored.
...
The Media Asset model supports future growth without redesign by allowing:

- Cloud storage provider changes.
- CDN delivery.
- Image optimization.
- Responsive image generation.
- Thumbnail generation.
- PDF catalogue hosting.
- Video galleries.
- 360-degree views.
- Future virtual tours.
- DAM-style tagging and search.
- Asset rights and usage tracking.
- CMS-managed media libraries.
- Media versioning and replacement workflows.
- Asset processing states.
- Multiple derivative files from a single original asset.

Future entities such as Asset Folder, Asset Collection, Digital Asset Management Library, Asset Processing Job, Storage Provider, CDN Configuration, Media Usage Record, and Media Analytics Event can reference Media Asset without changing the core model.
...
```json
{
  "id": "6b4d8f2a-3c91-4f72-9a18-5d7e2c41b601",
  "publicId": "media-sfg-modular-kitchen-catalogue-2026-pdf",
  "companyId": "2a7c91e5-4d8f-4c21-9f62-6e39d8a4f101",
  "name": "Modular Kitchen Catalogue 2026 PDF",
  "slug": "modular-kitchen-catalogue-2026-pdf",
  "description": "PDF document for the Modular Kitchen Catalogue 2026.",
  "assetType": "pdf",
  "mimeType": "application/pdf",
  "fileName": "modular-kitchen-catalogue-2026.pdf",
  "fileExtension": "pdf",
  "fileSizeBytes": 12450000,
  "checksum": "sha256-placeholder-value",
  "storageProvider": "cloudflareR2",
  "storageReference": {
    "provider": "cloudflareR2",
    "container": "star-furniture-goa-media",
    "objectKey": "catalogues/modular-kitchen-catalogue-2026.pdf",
    "region": null,
    "accessLevel": "publicRead"
  },
  "cdnUrl": "https://cdn.starfurnituregoa.com/catalogues/modular-kitchen-catalogue-2026.pdf",
  "sourceUrl": null,
  "altText": "Modular Kitchen Catalogue 2026 PDF by Star Furniture Goa",
  "caption": "Modular Kitchen Catalogue 2026",
  "creditText": null,
  "usageRights": {
    "licenseType": "owned",
    "usageScope": "website-and-marketing",
    "expiresAt": null
  },
  "width": null,
  "height": null,
  ...
  "dominantColor": null,
  "responsiveVariants": [],
  "thumbnailMediaAssetId": "media-modular-kitchen-catalogue-2026-thumbnail",
  "derivativeMediaAssetIds": [],
  "relatedMediaAssetIds": [
    "media-modular-kitchen-catalogue-2026-cover"
  ],
  "assetCategory": "catalogue",
  "tags": [
    "catalogue",
    "modular-kitchen",
    "pdf",
  ...
  "searchKeywords": [
    ...
    "Star Furniture Goa catalogue",
    "2026 kitchen designs"
  ],
  "seoMetadata": {
    "metaTitle": "Modular Kitchen Catalogue 2026 PDF",
    "metaDescription": "Download or view the Modular Kitchen Catalogue 2026 by Star Furniture Goa.",
    "canonicalUrl": null,
    "ogImageMediaReference": {
      "mediaAssetId": "media-modular-kitchen-catalogue-2026-cover",
      "mediaType": "image",
  ...
  "versionInformation": {
    "version": 1,
    "versionLabel": "Initial Upload",
    "versionStatus": "published",
    "effectiveFrom": "2026-07-11T00:00:00+05:30",
    "effectiveTo": null,
    "changeSummary": "Initial catalogue PDF media asset."
  },
  "metadata": {},
  "status": "active",
  "createdAt": "2026-07-11T00:00:00+05:30",
...
The Wishlist entity represents a customer's saved interest in Products, Product Variants, and future saved business items for later consideration.

A Wishlist does not represent an order, quotation, or enquiry.

It exists only to help anonymous visitors or future registered customers save items they may want to revisit, compare, share, or enquire about later.

A Wishlist must not own Product data directly.

Saved items must be represented through Wishlist Items using references to Products, Product Variants, Catalogues, Collections, or other supported entities.

---

### 2. Responsibilities

The Wishlist entity is responsible for:

- Storing a customer's saved interest.
- Supporting anonymous visitor wishlists.
- Supporting future registered customer wishlists.
- Referencing saved Products without embedding Product data.
- Referencing saved Product Variants when a specific configuration is selected.
- Supporting future saved Catalogues and Collections.
- Supporting wishlist sharing.
- Supporting wishlist-to-enquiry conversion.
- Supporting persistence across devices in the future.
- Supporting soft deletion and recovery.
- Supporting future analytics and customer intent insights.

The Wishlist entity is not responsible for:

- Owning Product data.
- Owning Product Variant data.
- Owning Catalogue or Collection data.
- Representing an order.
- Representing a quotation.
- Representing an enquiry.
- Storing pricing calculations.
- Storing customer account details directly.
- Replacing Customer Account or Enquiry entities.
...
| Name | Data Type | Required or Optional | Description | Validation Rules |
|---|---|---|---|---|
| `id` | UUID String | Required | Internal unique identifier for the Wishlist. | Must be globally unique and immutable. |
| `companyId` | UUID String | Required | Reference to the Company the Wishlist belongs to. | Must reference one valid Company. |
| `ownerType` | Controlled String | Required | Defines whether the Wishlist belongs to an anonymous visitor or a future registered customer. | Suggested values: `anonymousSession`, `customerAccount`. |
| `items` | Array of Wishlist Item Objects | Required | Saved items inside the Wishlist. | Must be an array. Can be empty. Each item must reference a supported business entity. |
| `status` | Controlled String | Required | Lifecycle status of the Wishlist. | Suggested values: `active`, `inactive`, `converted`, `archived`, `deleted`. |
| `createdAt` | ISO 8601 Date String | Required | Date and time when the Wishlist was created. | Must use ISO 8601 format. |
| `updatedAt` | ISO 8601 Date String | Required | Date and time when the Wishlist was last updated. | Must use ISO 8601 format and update whenever Wishlist data changes. |
...
| Name | Data Type | Required or Optional | Description | Validation Rules |
|---|---|---|---|---|
| `publicId` | String | Optional | Customer-safe identifier for sharing or future account access. | Must not expose internal sequencing or sensitive data. |
| `name` | String | Optional | Customer-defined Wishlist name. | Should be optional because anonymous wishlists may not be named. |
| `description` | String | Optional | Customer-defined or internal description of the Wishlist. | Must not contain Product data copied from referenced entities. |
| `anonymousSessionId` | String | Optional | Reference to the anonymous visitor session that owns the Wishlist. | Required when `ownerType` is `anonymousSession`. Must not contain sensitive personal data. |
| `customerAccountId` | UUID String | Optional | Future reference to a registered Customer Account. | Required when `ownerType` is `customerAccount`. Must reference a valid Customer Account when that entity exists. |
| `shareToken` | String | Optional | Secure token used for future wishlist sharing. | Must be unique, non-guessable, and safe for public URLs when used. |
| `sharingSettings` | Object | Optional | Controls whether and how the Wishlist can be shared. | Must not expose private customer information. |
| `isShareable` | Boolean | Optional | Indicates whether the Wishlist can be shared. | Must default to `false` unless sharing is intentionally enabled. |
| `sharedAt` | ISO 8601 Date String | Optional | Date and time when the Wishlist was first shared. | Must use ISO 8601 format when present. |
| `lastAccessedAt` | ISO 8601 Date String | Optional | Date and time when the Wishlist was last viewed or updated by the customer. | Must use ISO 8601 format when present. |
| `expiresAt` | ISO 8601 Date String | Optional | Expiry date for anonymous or temporary wishlists. | Must be later than `createdAt` when present. |
| `convertedEnquiryIds` | Array of UUID Strings | Optional | References to Enquiries created from this Wishlist. | Must reference valid Enquiry entities when used. |
| `convertedAt` | ISO 8601 Date String | Optional | Date and time when the Wishlist was converted into an Enquiry. | Must use ISO 8601 format when present. |
| `sourceContext` | Object | Optional | Non-critical context about how the Wishlist was created. | May include source page, campaign, or entry point. Must not replace analytics events. |
| `persistenceContext` | Object | Optional | Future persistence information for device syncing or account migration. | Must not store credentials or sensitive session data. |
| `analyticsContext` | Object | Optional | Future lightweight analytics context. | Must not contain sensitive customer information or replace formal analytics records. |
| `metadata` | Object | Optional | Non-critical extensibility data. | Must not contain Product data, customer account data, pricing logic, or enquiry details. |
| `isDeleted` | Boolean | Optional | Soft deletion flag. | Must default to `false` when used. |
| `deletedAt` | ISO 8601 Date String | Optional | Date and time when the Wishlist was soft deleted. | Required only when `isDeleted` is `true`. |

---

### Wishlist Item Object

Wishlist Item is not a standalone entity.

It belongs inside Wishlist because it has no independent business lifecycle outside the Wishlist.

| Name | Data Type | Required or Optional | Description | Validation Rules |
|---|---|---|---|---|
| `id` | UUID String | Required | Internal unique identifier for the Wishlist Item. | Must be unique within the Wishlist. |
| `itemType` | Controlled String | Required | Type of saved item. | Suggested values: `product`, `productVariant`, `catalogue`, `collection`. |
| `savedAt` | ISO 8601 Date String | Required | Date and time when the item was saved. | Must use ISO 8601 format. |
| `productId` | UUID String | Optional | Reference to a saved Product. | Required when `itemType` is `product` or `productVariant`. |
| `productVariantId` | UUID String | Optional | Reference to a saved Product Variant. | Required when `itemType` is `productVariant`. Must belong to the referenced Product. |
| `catalogueId` | UUID String | Optional | Future reference to a saved Catalogue. | Required when `itemType` is `catalogue`. |
| `collectionId` | UUID String | Optional | Future reference to a saved Collection. | Required when `itemType` is `collection`. |
| `quantityIntent` | Number | Optional | Optional customer interest quantity. | Must be a positive number when present. Must not imply an order. |
| `customerNote` | String | Optional | Optional note added by the customer. | Must not be used to store Product details copied from referenced entities. |
| `sourceContext` | Object | Optional | Context about where the item was saved from. | May include source page, section, or campaign. |
| `displayOrder` | Number | Optional | Customer-controlled or system-controlled ordering of Wishlist Items. | Must be a non-negative number when present. |
| `status` | Controlled String | Optional | Item-level status within the Wishlist. | Suggested values: `active`, `removed`, `converted`, `archived`. |
| `removedAt` | ISO 8601 Date String | Optional | Date and time when the item was removed from the Wishlist. | Required when item status is `removed`. |

---

### 5. Relationships

- Wishlist belongs to one Company through `companyId`.
- Wishlist may belong to an anonymous visitor session through `anonymousSessionId`.
- Wishlist may belong to a future Customer Account through `customerAccountId`.
- Wishlist contains Wishlist Items as embedded child objects.
- Wishlist Items reference Products through `productId`.
- Wishlist Items reference Product Variants through `productVariantId`.
- Wishlist Items may reference future Catalogues through `catalogueId`.
- Wishlist Items may reference future Collections through `collectionId`.
- Wishlist may reference Enquiries created from it through `convertedEnquiryIds`.

Wishlist does not store full Product, Product Variant, Catalogue, or Collection objects.

Wishlist does not own saved business entities.

---

### 6. Business Rules

- A Wishlist must belong either to an anonymous visitor session or to a future registered Customer Account.
- If `ownerType` is `anonymousSession`, `anonymousSessionId` must be present.
- If `ownerType` is `customerAccount`, `customerAccountId` must be present.
- A Wishlist must not represent an order, quotation, or enquiry.
- A Wishlist may be converted into an Enquiry, but it must remain a separate entity.
- Wishlist Items must reference saved business entities instead of embedding them.
- Wishlist must not duplicate Product, Product Variant, Catalogue, or Collection data.
- A Product Variant Wishlist Item must also reference its parent Product through `productId`.
- Removed Wishlist Items may be preserved for history or analytics when needed.
- Anonymous wishlists may expire based on `expiresAt`.
- Shared wishlists must not expose private customer or session information.
- Soft-deleted Wishlists must not appear in active customer experiences unless restored.
- Wishlist history should remain available for future analytics, enquiry context, and customer account migration where appropriate.

---

### 7. Shared Value Objects Used

No customer-facing shared value objects are required for the core Wishlist model.

The Wishlist entity follows the global standards for:
...
Wishlist intentionally does not use SEO Metadata, Media Reference, Address, Contact Information, Business Hours, or Geo Coordinates because those concepts do not describe saved customer interest.

---

### 8. Future Expansion

The Wishlist model supports future growth without redesign by allowing:

- Registered customer ownership through `customerAccountId`.
- Anonymous-to-account migration.
- Cross-device persistence.
- Wishlist sharing through `shareToken` and `sharingSettings`.
- Saved Catalogues.
- Saved Collections.
- Saved Product Variants.
- Wishlist-to-Enquiry conversion.
- Wishlist-to-Quotation conversion.
- Future customer dashboards.
- Future analytics and recommendation workflows.
- Future abandoned-interest insights.
- Future collaboration or shared design boards.

Future entities such as Customer Account, Enquiry, Quotation, Analytics Event, Recommendation, and Lead Pipeline can reference Wishlist without changing the core Wishlist structure.
...
```json
{
  "id": "1c8f92a7-6d43-4a81-8f25-9e3b7d1a5402",
  "publicId": "wishlist-sfg-guest-001",
  "companyId": "2a7c91e5-4d8f-4c21-9f62-6e39d8a4f101",
  "name": "Saved Furniture Ideas",
  "description": null,
  "ownerType": "anonymousSession",
  "anonymousSessionId": "anon-session-9f8d7c6b5a",
  "customerAccountId": null,
  "items": [
    {
      "id": "wishlist-item-01",
      "itemType": "product",
      "productId": "product-premium-modular-kitchen",
      "productVariantId": null,
      "catalogueId": null,
      "collectionId": null,
      "quantityIntent": 1,
      "customerNote": "Interested in a similar layout for a compact kitchen.",
      "sourceContext": {
        "sourcePage": "product-detail",
        "sourceSection": "featured-products"
      },
      "displayOrder": 1,
      "status": "active",
      "savedAt": "2026-07-11T00:00:00+05:30",
      "removedAt": null
    },
    {
      "id": "wishlist-item-02",
      ...
      "collectionId": null,
      "quantityIntent": 1,
      "customerNote": "Check if this can be customized with a mirror door.",
      "sourceContext": {
        "sourcePage": "product-detail",
        "sourceSection": "variant-selector"
      },
      "displayOrder": 2,
      "status": "active",
      "savedAt": "2026-07-11T00:05:00+05:30",
      "removedAt": null
    }
  ],
  "shareToken": null,
  "sharingSettings": {
    ...
    "allowEnquiryFromSharedLink": false
  },
  "isShareable": false,
  "sharedAt": null,
  "lastAccessedAt": "2026-07-11T00:10:00+05:30",
  "expiresAt": "2026-08-10T00:00:00+05:30",
  "convertedEnquiryIds": [],
  "convertedAt": null,
  "sourceContext": {
    "createdFrom": "website",
  ...
  "analyticsContext": {
    "itemCount": 2,
    "lastItemAddedAt": "2026-07-11T00:05:00+05:30"
  },
  "metadata": {},
  "status": "active",
  "createdAt": "2026-07-11T00:00:00+05:30",
  "updatedAt": "2026-07-11T00:10:00+05:30",
  "isDeleted": false,
  "deletedAt": null
}
```
